...
import { accountCaseId, caseDrillEntitlement, caseEntitlement, getCase, latestProfile, pastSessions } from "@/lib/server/repo";
...
export default async function Home(props: PageProps<"/app">) {
  ...
  const caseRow = caseId ? await getCase(supabase, caseId) : null;
  if (!caseId || !caseRow) {
    return (
  ...
  return (
    <>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
          ...
          <Card>
            ...
            {!current ? (
              <p className="mt-6 text-sm">
                First, <Link className="underline" href={`/app/profile`}>confirm your facts</Link>. The officer only uses what you confirm.
              </p>
            ) : ent.kind === "none" ? (
              <>
                ...
                <Link href={`/app/pass`} className="mt-6 inline-block rounded-[3px] bg-ink px-5 py-2.5 text-sm font-semibold text-on-ink hover:bg-stamp">
                  Get interviews
                </Link>
              </>
            ) : (
        ...
        <div className="space-y-6">
          {current && <DailyPlanCard plan={plan} hasDate={Boolean(caseRow.interview_at)} />}
          {current && (
            <Card>
              <h2 className="font-display text-2xl uppercase">Guides</h2>
              <ul className="mt-3 space-y-2 text-sm">
                <li>
                  <Link className="underline underline-offset-2" href={`/app/facts`}>Know your file</Link>
                  <span className="text-muted">: the facts and numbers on the officer&rsquo;s screen, with a quick quiz</span>
                </li>
                <li>
                  <Link className="underline underline-offset-2" href={`/app/progress`}>Progress</Link>
                  <span className="text-muted">: your readiness over time, topic by topic</span>
                </li>
                <li>
                  <Link className="underline underline-offset-2" href={`/app/social`}>Social media check</Link>
                  <span className="text-muted">
                    {caseRow.visa_type === "F1" ? ": required public profiles, checked against your application" : ": do your profiles match your application?"}
                  </span>
                </li>
                <li>
                  <Link className="underline underline-offset-2" href={`/app/day`}>Interview day</Link>
                ...
                {(current.profile.history.priorRefusals.length > 0 || outcome?.result === "refused_214b" || outcome?.result === "refused_other") && (
                  <li>
                    <Link className="underline underline-offset-2" href={`/app/refused`}>Refused before</Link>
