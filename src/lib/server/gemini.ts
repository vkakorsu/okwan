...
DS-160: set ds160Part to "confirmation_page" if this is only the one-page confirmation (barcode, confirmation number, photo) without the answers, otherwise "full_answers". The full answers hold most of the case. Fill what it states: full name, date of birth (YYYY-MM-DD), nationality, marital status, address city, intended arrival date (YYYY-MM-DD) and length of stay, US address or where they'll stay (visit.stayingAt), who is paying (funding.sponsors with relationship, name, occupation and employer or business), travel companions, previous US visits (history.usVisits: year and length of stay in days for each; priorUsVisits is their count), previous refusals, relatives in the US (usContacts, with status), father's, mother's and spouse's occupations (family), present employer, role, monthly income and start date, previous education (education.lastSchool, lastProgram, graduationYear), and countries visited in the last five years.
Passport bio page (the page with the photo): applicant.fullName (given names then surname, as printed), applicant.dateOfBirth and applicant.passportExpiry (YYYY-MM-DD), applicant.nationality, and passportLast4: only the last 4 characters of the passport number, never the whole number.
